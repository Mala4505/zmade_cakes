...
export default async function OrderPage() {
  ...
  const [flavorsRes, sizesRes, occasionsRes, themesRes, decorationsRes, blackoutsRes, phoneRow] = await Promise.all([
    ...
    supabase.from('theme_options').select('id, name').eq('is_active', true).order('sort_order'),
    supabase.from('decoration_style_options').select('id, name').eq('is_active', true).order('sort_order'),
